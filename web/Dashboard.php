...
if ((!empty($select_vendor_table["bank_code"]) && is_numeric($select_vendor_table["bank_code"]) && !empty($select_vendor_table["bvn"]) && is_numeric($select_vendor_table["bvn"]) && strlen($select_vendor_table["bvn"]) == 11) || (!empty($get_logged_user_details["bank_code"]) && is_numeric($get_logged_user_details["bank_code"]) && !empty($get_logged_user_details["nin"]) && is_numeric($get_logged_user_details["nin"]) && strlen($get_logged_user_details["nin"]) == 11)) {
	...
	$virtual_bank_code_arr = array("232", "035", "50515", "120001", "100039", "110072", "GLOBUS BANK", "WEMA BANK");
	...
	if ((getUserVirtualBank() == false) || ((is_array(getUserVirtualBank()) == true) && (!empty(array_diff($virtual_bank_code_arr, $registered_virtual_bank_arr))))) {
		...
		//Fincra Admin/User BVN Virtual Account Generation
		if ((!empty($select_vendor_table["bvn"]) && is_numeric($select_vendor_table["bvn"]) && strlen($select_vendor_table["bvn"]) == 11) || (!empty($get_logged_user_details["bvn"]) && is_numeric($get_logged_user_details["bvn"]) && strlen($get_logged_user_details["bvn"]) == 11)) {
			$select_fincra_gateway_details = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sas_payment_gateways WHERE vendor_id='" . $get_logged_user_details["vendor_id"] . "' && gateway_name='fincra' LIMIT 1"));
			$user_fincra_account_reference = str_replace([".", "-", ":"], "", $_SERVER["HTTP_HOST"]) . "-" . $get_logged_user_details["username"] . "-" . $get_logged_user_details["email"];

			foreach (["globus", "wema"] as $bank_id) {
				$fincra_create_reserve_account_array = array("accountType" => "individual", "currency" => "NGN", "channel" => $bank_id, "KYCInformation" => array("email" => $user_fincra_account_reference, "firstName" => $get_logged_user_details["firstname"], "lastName" => $get_logged_user_details["lastname"]));
				if (strlen($select_vendor_table_bvn) === 11) {
					$fincra_create_reserve_account_array["KYCInformation"]["bvn"] = $select_vendor_table_bvn;
				}
				if (strlen($select_vendor_table_nin) === 11) {
					$fincra_create_reserve_account_array["KYCInformation"]["nin"] = $select_vendor_table_nin;
				}
				$get_fincra_reserve_account = json_decode(makeFincraRequest("post", $select_fincra_gateway_details["secret_key"], "profile/virtual-accounts/requests", $fincra_create_reserve_account_array), true);

				if ($get_fincra_reserve_account["status"] == "success") {
					$fincra_reserve_account_response = json_decode($get_fincra_reserve_account["json_result"], true);
					addUserVirtualBank($fincra_reserve_account_response["data"]["_id"], $fincra_reserve_account_response["data"]["accountInformation"]["bankName"], $fincra_reserve_account_response["data"]["accountInformation"]["bankName"], $fincra_reserve_account_response["data"]["accountInformation"]["accountNumber"], $fincra_reserve_account_response["data"]["accountInformation"]["accountName"]);
					//$virtual_account_vaccount_err .= '<span class="color-4">Virtual Account Created Successfully</span>';
				}

				if ($fincra_reserve_account_response["status"] == "failed") {

					//$virtual_account_vaccount_err .= '<span class="color-4">'.$get_fincra_access_token["message"].'</span>';
				}
			}
		}
		...
		<div class="row">
			...
			<div class="col-12 col-md-6 col-lg-3">
				<div class="card info-card card-blue shadow">
					<div class="card-body">
						<h5 class="card-title"><?php echo $get_logged_user_details["username"]; ?> <span>| Account
								Type</span></h5>
		...
		<div class="row">
			...
			<div class="col-6 col-lg-3">
				<div class="card info-card sales-card p-3 align-items-center">
					<a href="Fund.php" class="">
						<h5 class="card-title">Wallet Funding</h5>
					</a>
				</div>
			</div>
			<!-- Col End -->
			...
			<div class="col-6 col-lg-3">
				<div class="card info-card sales-card p-3 align-items-center">
					<a href="SubmitPayment.php" class="">
		...
		<div class="row d-flex justify-content-center justify-content-between">
			...
			<div class="col-lg-12">
				...
				<div class="col-12 col-lg-12">
					<div class="card info-card sales-card p-3 align-items-center">
						<h5 class="card-title">
							Refer and Earn Big, copy referral link <button class="btn btn-primary rounded-circle"
								onclick="copyReferLink();" title="Click To Copy">Copy Link</button>
						</h5>
					</div>
				</div>
				<!-- Col End -->
				...
				<script>
					...
					const copyReferLink = async () => {
						try {
							...
							// simple toast/alert feedback
							Swal.fire({
								icon: 'success',
								title: 'Copied!',
								text: 'Referral link copied: ' + ReferLink,
								timer: 2000,
									...
									foreach (getUserVirtualBank() as $bank_accounts_json) {
										...
										if (in_array($bank_accounts_json["bank_code"], array("110072", "232", "035", "50515", "120001", "100039", "GLOBUS BANK", "WEMA BANK"))) {
											...
											<div class="card shadow-sm flex-shrink-0"
												style="min-width: 100%; max-width: 100%; border-radius: 12px;">

												<div class="card-header bg-success text-white text-center fw-semibold py-2"
													style="font-size: 14px;">
													<?php echo strtoupper($bank_accounts_json["account_name"]); ?>
												</div>

												<div class="card-body bg-light text-center p-2">
													<h6 class="text-success mb-1" style="font-size: 13px;">
														<?php echo strtoupper($bank_accounts_json["bank_name"]); ?>
													</h6>
													<h5 class="text-dark fw-bold mb-1" style="font-size: 16px;">
														<?php echo $bank_accounts_json["account_number"]; ?>
														<span class="ms-1" style="cursor:pointer"
															onclick="copyAccount('<?php echo $bank_accounts_json['account_number']; ?>')">
															<i class="bi bi-copy text-success small"
																title="Copy Account Number"></i>
														</span>
													</h5>
													<div class="text-muted" style="font-size: 13px;">
														Charges <span class="fw-bold text-danger ms-1">₦50</span>
													</div>
												</div>
											</div>
											<?php
				...
				<div class="card info-card sales-card p-3 align-items-center">
					<div class="card-body col-12 align-items-center">
						...
						<form method="post" action="">
							<select id="" name="upgrade-type" onchange="" class="form-select col-12" required />
							...
							<button id="" name="upgrade-user" type="submit" class="btn btn-success col-12">
								Proceed
							</button>
						</form>
					</div>
