...
<head>
    <title>Virtual Card Transactions | <?php echo $get_all_site_details["site_title"]; ?></title>
    <meta charset="UTF-8" />
...
<body>
    ...
    <div class="pagetitle">
        <h1>VIRTUAL CARD TRANSACTIONS</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Virtual Card Transactions</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section dashboard">
        <div class="col-12">
            ...
            if (isset($_GET["searchq"]) && !empty(trim(strip_tags($_GET["searchq"])))) {
                $search_statement = " && (product_unique_id LIKE '%" . trim(strip_tags($_GET["searchq"])) . "%' OR reference LIKE '%" . trim(strip_tags($_GET["searchq"])) . "%' AND (type_alternative LIKE '%Mastercard%' OR type_alternative LIKE '%Visa%' OR type_alternative LIKE '%Verve%') OR description LIKE '%" . trim(strip_tags($_GET["searchq"])) . "%')";
            ...
            <div class="card info-card px-5 py-5">
                <div class="row">
                    <form method="get" action="Transactions.php" class="">
                        <input style="user-select: auto;" name="searchq" type="text"
                            ...
                            placeholder="Reference No, Meter No, Phone Number e.t.c" class="form-control mt-3" />
                ...
                <div class="mt-2 justify-content-between justify-items-center">
                    <?php if (isset($_GET["page"]) && is_numeric(trim(strip_tags($_GET["page"]))) && (trim(strip_tags($_GET["page"])) > 1)) { ?>
                        <a
                            href="Transactions.php?<?php echo $search_parameter; ?>page=<?php echo (trim(strip_tags($_GET["page"])) - 1); ?>">
