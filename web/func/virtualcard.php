...
$allowed_kyc_array = array("nigerian_bvn", "nigerian_nin");
if (in_array($purchase_method, $purchase_method_array)) {
    if ($purchase_method === "WEB") {
        ...
        $selfie_url = mysqli_real_escape_string($connection_server, trim(strip_tags($_POST["selfie-dataurl"])));
    }
    ...
    if (in_array($purchase_method, array("API", "APP"))) {
        ...
        $selfie_url = mysqli_real_escape_string($connection_server, trim(strip_tags($get_api_post_info["selfie_url"])));
    }
    ...
    if (in_array($type, $data_type_array)) {
        if (!empty(userBalance(1)) && is_numeric(userBalance(1)) && (userBalance(1) > 0)) {
            ...
            if (!empty($isp) && !empty($qty_number) && is_numeric($qty_number) && ($qty_number >= 1) && !empty($type) && !empty($quantity) && !empty($firstname) && !empty($lastname) && !empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($phone) && !empty($address) && !empty($postal_code) && is_numeric($postal_code) && !empty($state) && !empty($country) && in_array($country, $allowed_country_array) && !empty($kyc_mode) && in_array($kyc_mode, $allowed_kyc_array) && !empty($kyc_id) && !empty($selfie_url)) {
