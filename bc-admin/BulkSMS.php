<?php session_start();
    ...
    if(isset($_POST["install-product"])){
        ...
        $product_varieties = array(
            ...
            "glo" => array("standard_sms", "flash_sms", "in_app_otp"),
            "9mobile" => array("standard_sms", "flash_sms", "in_app_otp")
        );
...
<body>
    ...
    <section class="section dashboard">
      <div class="col-12">
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            ...
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="container">
                    <span id="user-status-span" class="h5" style="user-select: auto;">BULK SMS STATUS</span>
                </div><br/>
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            <span style="user-select: auto;" class="h4 fw-bold">INSTALLED BULK SMS STATUS</span><br>
                        ...
                        if(mysqli_num_rows($select_item_lists) >= 1){
                            while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                ...
                                '<tr>
                                    ...
                                    <td>'.render_action_buttons($list_details["product_name"], "bulk-sms", $list_details["status"]).'</td>
                                </tr>';
                            }
                        }
                    ?>
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            <span style="user-select: auto;" class="h4 fw-bold">BULK SMS DISCOUNT</span><br>
            <div style="user-select: auto; cursor: grab;" class="overflow-auto mt-1">
              <table style="" class="table table-responsive table-striped table-bordered" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                <thead class="thead-dark">
                  <tr>
                                ...
                                if((mysqli_num_rows($get_api_lists) == 1) && (mysqli_num_rows($product_smart_table) > 0) && (mysqli_num_rows($product_agent_table) > 0) && (mysqli_num_rows($product_api_table) > 0)){
                                    while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                        ...
                                        if ($product_smart_details['status'] == 1) {
                                            $actions .= '<a href="BulkSMS.php?action=disable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-warning btn-sm mb-1 d-block">Disable</a> ';
                                        } else {
                                            $actions .= '<a href="BulkSMS.php?action=enable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-success btn-sm mb-1 d-block">Enable</a> ';
                                        }
                                        $actions .= '<a href="BulkSMS.php?action=delete&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-danger btn-sm d-block" onclick="return confirm(\'Are you sure you want to delete this package? This action cannot be undone.\');">Delete</a>';
...
",$csv_price_level_array); ?>`, "bulk-sms.csv");' style="text-decoration: underline; user-select: auto;" class="h5 color-2 outline-none text-bold-600">Download Price CSV</a>
           </div>
          </div>
        </div>
      </div>
