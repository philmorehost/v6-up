...
<body>
    ...
    <section class="section dashboard">
      <div class="col-12">
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            ...
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="container">
                    <span id="user-status-span" class="h5" style="user-select: auto;">CORPORATE DATA STATUS</span>
                </div><br/>
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            <span style="user-select: auto;" class="h4 fw-bold">INSTALLED CORPORATE DATA STATUS</span><br>
                        ...
                        if(mysqli_num_rows($select_item_lists) >= 1){
                            while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                ...
                                '<tr>
                                    ...
                                    <td>'.render_action_buttons($list_details["product_name"], "cg-data", $list_details["status"]).'</td>
                                </tr>';
                            }
                        }
                    ?>
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3">
            <span style="user-select: auto;" class="h4 fw-bold">CORPORATE DATA DISCOUNT</span><br>
            <div style="user-select: auto; cursor: grab;" class="overflow-auto mt-1">
              <table style="" class="table table-responsive table-striped table-bordered" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                <thead class="thead-dark">
                  <tr>
                            ...
                            foreach($item_name_array_2 as $products){
                                ...
                                if((mysqli_num_rows($get_api_lists) == 1) && (mysqli_num_rows($product_smart_table) > 0) && (mysqli_num_rows($product_agent_table) > 0) && (mysqli_num_rows($product_api_table) > 0)){
                                    while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                        ...
                                        $toggle_link = "CorporateData.php?action=toggle_status&api_id={$product_smart_details['api_id']}&product_id={$product_smart_details['product_id']}&val_1=" . urlencode($product_smart_details['val_1']) . "&current_status={$product_smart_details['status']}";
                                        ...
                                        echo 
                                            '<tr style="background-color: transparent !important;">
                                                ...
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_corporate_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_smart_level" name="smart-price[]" type="text" value="'.$product_smart_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="product-price form-control mb-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_corporate_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_agent_level" name="agent-price[]" type="text" value="'.$product_agent_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="product-price form-control mb-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_corporate_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" name="api-price[]" type="text" value="'.$product_api_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="product-price form-control mb-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_corporate_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_days" name="product-days[]" type="text" value="'.$product_api_details["val_3"].'" placeholder="Days" pattern="[0-9.]{1,}" title="Days Must Be A Digit" class="form-control mb-1" required/>
                                                </td>
                                                <td>'.$status_badge.'</td>
        ...
        <div class="card info-card px-5 py-5">
          <div class="row mb-3"> 
            ...
            <a onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "corporate-data.csv");' style="text-decoration: underline; user-select: auto;" class="h5 text-danger mt-3">Download Price CSV</a>
           </div>
          </div>
        </div>
      </div>
